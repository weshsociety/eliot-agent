...
memory = Memory(graph_file="data/octopus_data.json")
...
@app.route("/api/moleskine", methods=["POST"])
def api_moleskine_write():
    """Agents IA POST découvertes"""
    entry = request.json
    if not entry or not entry.get("content"):
        return jsonify({"error": "content_required"}), 400
    
    entry["timestamp"] = datetime.datetime.now().isoformat()
    entry["agent_author"] = entry.get("agent_author", "anonyme")
    ...
    return jsonify({"status": "persisted", "entry_id": entry.get("entry_id")}), 201

@app.route("/api/moleskine", methods=["GET"])
def api_moleskine_read():
    """Agents IA READ mémoire collective"""
    try:
        with open(MOLESKINE_PATH, "r", encoding="utf-8") as f:
            entries = json.load(f)
    except:
